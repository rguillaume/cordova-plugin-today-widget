...
module.exports = function(context) {
  ...
  var ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES = getCordovaParameter(
    "WIDGET_ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES",
    contents
  );
  var SWIFT_VERSION = getCordovaParameter("SWIFT_VERSION", contents);
  var DEBUG_PROVISIONING_PROFILE = getCordovaParameter(
    "WIDGET_DEBUG_PROVISIONING_PROFILE",
    contents
  );
  var RELEASE_PROVISIONING_PROFILE = getCordovaParameter(
    "WIDGET_RELEASE_PROVISIONING_PROFILE",
    contents
  );
            ...
            if (productName.indexOf(widgetName) >= 0) {
              ...
              if (
                ...
                buildSettingsObj["CODE_SIGN_IDENTITY"] =
                  '"iPhone Distribution"';
                log(
                  "Added signing identity to release build settings!",
                  "info"
                );
              }
            }
